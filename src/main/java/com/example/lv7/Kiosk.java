...
public class Kiosk<T extends Number> {
    ...
    public void start() {
        ...
        while (true) {
            try {
                ...
                if (printCartMenu(choiceCategory)) continue;
                if (printOrderCancel(choiceCategory)) continue;

                printMenuSelection(choiceCategory);
    ...
    private boolean printCartMenu(int choiceCategory) {
        // 조회
        // 장바구니에 담긴 메뉴가 있고, 입력값 == [메뉴종류의 개수 + 1]
        if (!cart.getCartList().isEmpty() && choiceCategory == menus.size() + 1) {
    ...
    private boolean printOrderCancel(int choiceCategory) {
        if (!cart.getCartList().isEmpty() && choiceCategory == menus.size() + 2) {
    ...
    // 할인 정보 출력
    private void printOrderProcess() {
        System.out.println("\n할인 정보를 입력해주세요.\n");
    ...
    private void printMenuSelection(int choiceCategory) {
